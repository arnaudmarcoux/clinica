...
class t1_volume_longitudinal(cpe.Pipeline):
    ...
    Example:
        >>> from t1_volume_longitudinal import t1_volume_longitudinal
        >>> pipeline = t1_volume_longitudinal('~/MYDATASET_BIDS', '~/MYDATASET_CAPS')
        >>> pipeline.parameters = {
        >>>     # ...
        >>> }
    ...
    def build_core_nodes(self):
        ...
        # important paths
        pipelineFolderPath = '/Users/emmaducos/Desktop/clinica/clinica/pipelines/t1_volume_longitudinal'
        matlabToolboxPath = '/Users/emmaducos/Documents/MATLAB'

        # Serial Longitudinal Registration SPM12
        # ======================================
        # from sesbaseline_T1w and seslast_T1w to get avg_T1w for segmentation
        # from sesbaseline_FLAIR and seslast_FLAIR to get avg_FLAIR for lesion segmentation
        ...
        serialLongitudinalRegistration_T1w_nd.inputs.pipelineFolder = pipelineFolderPath
        serialLongitudinalRegistration_T1w_nd.inputs.nameOfNode = 'T1w'
        serialLongitudinalRegistration_T1w_nd.inputs.working_directory = self.base_dir
        ...
        serialLongitudinalRegistration_FLAIR_nd.inputs.pipelineFolder = pipelineFolderPath
        serialLongitudinalRegistration_FLAIR_nd.inputs.nameOfNode = 'FLAIR'
        serialLongitudinalRegistration_FLAIR_nd.inputs.working_directory = self.base_dir

        # Segmentation SPM12 new
        # ===============================
        # from avg_T1w to get c3, c4, c5, c6 compartment segmentation
        # uses the nipype spm interface NewSegment
        # inputs extracted from a matlab SPM12 job script
        ...
        tissue1 = ((matlabToolboxPath + '/spm12/tpm/TPM.nii', 1), 1, (False, False), (False, False))
        tissue2 = ((matlabToolboxPath + '/spm12/tpm/TPM.nii', 2), 1, (False, False), (False, False))
        tissue3 = ((matlabToolboxPath + '/spm12/tpm/TPM.nii', 3), 2, (True, False), (False, False))
        tissue4 = ((matlabToolboxPath + '/spm12/tpm/TPM.nii', 4), 3, (True, False), (False, False))
        tissue5 = ((matlabToolboxPath + '/spm12/tpm/TPM.nii', 5), 4, (True, False), (False, False))
        tissue6 = ((matlabToolboxPath + '/spm12/tpm/TPM.nii', 5), 2, (True, False), (False, False))
        ...
        segmentCAT12sesbaseline_nd.inputs.pipelineFolder = pipelineFolderPath
        segmentCAT12sesbaseline_nd.inputs.working_directory = self.base_dir
        segmentCAT12sesbaseline_nd.inputs.matlabToolboxPath = matlabToolboxPath
        ...
        segmentCAT12seslast_nd.inputs.pipelineFolder = pipelineFolderPath
        segmentCAT12seslast_nd.inputs.working_directory = self.base_dir
        segmentCAT12seslast_nd.inputs.matlabToolboxPath = matlabToolboxPath
        ...
        segmentCAT12avg_nd.inputs.pipelineFolder = pipelineFolderPath
        segmentCAT12avg_nd.inputs.working_directory = self.base_dir
        segmentCAT12avg_nd.inputs.matlabToolboxPath = matlabToolboxPath
        ...
        segmentFsl_nd = npe.MapNode(fsl.FIRST(),
                                    name='segmentFsl',
                                    iterfield=['in_file'])
        segmentFsl_nd.inputs.out_file = 'avg_centered_' + self.subjects[0] + '_T1w'  # careful with self.subjects
                                                                            # because of several "same" subject names
        segmentFsl_nd.inputs.method = 'fast'
        # segmentFsl_nd.inputs.verbose = True
        # segmentFsl_nd.inputs.output_type = 'NIFTI' # gives an error
                                                # because of the FIRSTOutputSpec that looks only for .nii.gz filename

        # lesion segmentation toolbox
        # ===============================
        # wrap a matlabbatch SPM12 job script
        LSTLGA_nd = npe.Node(niu.Function(input_names=['avg_T1w', 'avg_FLAIR', 'working_directory', 'pipelineFolder'],
                                          output_names=['lesion_map'],
                                          function=utils.runLesionSegmentationToolboxLGA),
                             name='LSTLGA')
        LSTLGA_nd.inputs.working_directory = self.base_dir
        LSTLGA_nd.inputs.pipelineFolder = pipelineFolderPath
        ...
        imCalc_c1_nd.inputs.output = 'c1bin_avg_' + self.subjects[0] + '_T1w'  # carefull with self.subjects
        imCalc_c1_nd.inputs.expression = '(((i1>i2).*(i1>i3).*(i1>i4).*(i1>i5).*(i1>i6))-i7)'
        imCalc_c1_nd.inputs.pipelineFolder = pipelineFolderPath
        imCalc_c1_nd.inputs.working_directory = self.base_dir
        imCalc_c1_nd.inputs.nameOfNode = 'c1'
        imCalc_c1_nd.inputs.segmentation_file = None
        # create c2
        imCalc_c2_nd = imCalc_nd.clone('imCalc_c2')
        imCalc_c2_nd.inputs.output = 'c2bin_avg_' + self.subjects[0] + '_T1w'  # carefull with self.subjects
        imCalc_c2_nd.inputs.expression = '(((i2>i1).*(i2>i3).*(i2>i4).*(i2>i5).*(i2>i6))-i7)>0'
        imCalc_c2_nd.inputs.pipelineFolder = pipelineFolderPath
        imCalc_c2_nd.inputs.working_directory = self.base_dir
        imCalc_c2_nd.inputs.nameOfNode = 'c2'
        imCalc_c2_nd.inputs.segmentation_file = None
        # create c3
        imCalc_c3_nd = imCalc_nd.clone('imCalc_c3')
        imCalc_c3_nd.inputs.output = 'c3bin_avg_' + self.subjects[0] + '_T1w'  # carefull with self.subjects
        imCalc_c3_nd.inputs.expression = '(((i3>i1).*(i3>i2).*(i3>i4).*(i3>i5).*(i3>i6)-i7)>0)'
        imCalc_c3_nd.inputs.pipelineFolder = pipelineFolderPath
        imCalc_c3_nd.inputs.working_directory = self.base_dir
        imCalc_c3_nd.inputs.nameOfNode = 'c3'
        imCalc_c3_nd.inputs.segmentation_file = None
        # create c4
        imCalc_c4_nd = imCalc_nd.clone('imCalc_c4')
        imCalc_c4_nd.inputs.output = 'c4bin_avg_' + self.subjects[0] + '_T1w'  # carefull with self.subjects
        imCalc_c4_nd.inputs.expression = '(i4>i1).*(i4>i2).*(i4>i3).*(i4>i5).*(i4>i6)'
        imCalc_c4_nd.inputs.pipelineFolder = pipelineFolderPath
        imCalc_c4_nd.inputs.working_directory = self.base_dir
        ...
        imCalc_c5_nd.inputs.output = 'c5bin_avg_' + self.subjects[0] + '_T1w'  # carefull with self.subjects
        imCalc_c5_nd.inputs.expression = '(i5>i1).*(i5>i2).*(i5>i3).*(i5>i4).*(i5>i6)'
        imCalc_c5_nd.inputs.pipelineFolder = pipelineFolderPath
        ...
        imCalc_c6_nd.inputs.pipelineFolder = pipelineFolderPath
        ...
        imCalc_c1h_nd.inputs.output = 'c1hyb_avg_' + self.subjects[0] + '_T1w'  # carefull with self.subjects
        imCalc_c1h_nd.inputs.expression = \
            '(i1+(((i2<13.1).*(i2>9.9))+((i2<26.1).*(i2>25.9))+((i2<52.1).*(i2>48.9))+((i2<58.1).*(i2>57.9))))>0'
        imCalc_c1h_nd.inputs.pipelineFolder = pipelineFolderPath
        ...
        imCalc_c2h_nd.inputs.pipelineFolder = pipelineFolderPath
        ...
        # oldSegmentSPM12_nd.inputs.mask_image = '' # car matlabbatch{1}.spm.tools.oldseg.opts.msk = {''};

        # deformation in SPM12: creates a deformation field from a *_sn.mat file
        # ======================================================================
        # wrapper matlab for a first spatial deformation for spatial normalisation of T1w
        ...
        def_snmat_nd.inputs.working_directory = self.base_dir
        def_snmat_nd.inputs.pipelineFolder = pipelineFolderPath
